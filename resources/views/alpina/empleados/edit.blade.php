...
@section('content')
    @component('admin.components.panel')
        @slot('title', 'Modificar Empleado')
        {!! Form::model($empleado, [
            'route' => ['admin.empleados.update', $empleado],
            'method' => 'PUT',
            'id' => 'form_modificar_empleado',
            'class' => 'form-horizontal form-label-left',
            'novalidate'
        ])!!}
        @include('alpina.empleados._form')
        <div class="ln_solid"></div>
        <div class="form-group">
            <div class="col-md-6 col-md-offset-3">

                {{ link_to_route('admin.empleados.index',"Cancelar", [], ['class' => 'btn btn-info']) }}
                {!! Form::submit('Modificar Empleado', ['class' => 'btn btn-success']) !!}
            </div>
        </div>
        {!! Form::close() !!}
    @endcomponent
...
@push('functions')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var form = $('#form_modificar_empleado');
            $(form).parsley({
                trigger: 'change',
            ...
            form.submit(function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response, NULL, jqXHR) {
                        sessionStorage.setItem('update', 'El Empleado se ha modificado exitosamente.');
